	...
	public function setUsuario()
	{
		if ($_POST) {
			if (empty($_POST['txtIdentificacion']) || empty($_POST['txtNombre']) || empty($_POST['txtApellido']) || empty($_POST['txtTelefono']) || empty($_POST['txtEmail']) || empty($_POST['listRolid']) || empty($_POST['listStatus'])) {
				...
				if ($idUsuario == 0) {
					$option = 1;
					$strPassword =  empty($_POST['txtPassword']) ? encryption(passGenerator()) : encryption($_POST['txtPassword']);
					...
					$option = 2;
					$strPassword = empty($_POST['txtPassword']) ? $_POST['pass_verif'] : encryption($_POST['txtPassword']);
				...
				if ($request_user > 0) {
					if ($option == 1) {
						//validar envío de contraseña al correo del usuario registrado
						if (!empty($strPassword) && !empty($strEmail) && $request_user != false) {
							$nombreUsuario = $strNombre . ' ' . $strApellido;

							$dataCliente = array(
								'nombreUsuario' => $nombreUsuario,
								'email' => $strEmail,
								'asunto' => 'Envío Contraseña generada - ' . NOMBRE_REMITENTE,
							...
							if ($sendEmail) {
								...
								$arrResponse = array("status" => true, "msg" => 'Datos guardados correctamente pero no fue posible enviar la contraseña al correo del usuario.');
							}
						}
					} else {
		...
		if ($_POST) {
			if (empty($_POST['txtIdentificacion']) || empty($_POST['txtNombre']) || empty($_POST['txtApellido']) || empty($_POST['txtTelefono'])) {
				...
				if (!empty($_POST['txtPassword'])) {
					$strPassword = encryption($_POST['txtPassword']);
				}
